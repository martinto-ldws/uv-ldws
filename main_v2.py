...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def fit_polynomial(self, original_img, binary_warped, Minv):
        ...
        # Call the monitoring function if strict validation is enabled
        ...
        if left_detected:
            ...
            cv2.polylines(color_warp, np.int32([left_line_pts]), isClosed=False, color=(0, 255, 0), thickness=60)

        if right_detected:
            right_line_pts = np.array([np.transpose(np.vstack([right_fitx, ploty]))])
            cv2.polylines(color_warp, np.int32([right_line_pts]), isClosed=False, color=(0, 255, 0), thickness=60)
